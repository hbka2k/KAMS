    ...
    $(document).ready(function () {
        ...
        /*$("#btn_select_del").click(function(){
            ...
            if(confirm("선택한 데이터를 삭제 하시겠습니까?.")) {
                ...
                $.ajax({
                    url: '/artmore_adm/ajax/admin_list_delete.do',
                    type: 'POST',
                    dataType: 'json',
                    data: data,
                    xhrFields: {
        ...
        $('#btn_excel').click(function() {
            ...
            document.location.href="/artmore_adm/cont/excel/admin_list.do?" + param;
        });
        /////////////////////////////////////////////////////////////////
    });
...
<div class="container">
    ...
    <form name="frm" id="frm" method="post" action="/artmore_adm/cont/basic/admin_list.do">
    ...
    <article class="contUnit sch_area">
        ...
        <div class="line">
            ...
            <div class="formCont">
                ...
                <select name="searchKind4" id="searchKind4">
                    <option value="ALL" <c:if test="${searchKind4 eq 'ALL'}">selected="selected"</c:if>>전체</option>
                    <option value="1" <c:if test="${searchKind4 eq '1'}">selected="selected"</c:if>>성명</option>
                    <option value="2" <c:if test="${searchKind4 eq '2'}">selected="selected"</c:if>>아이디</option>
                </select>
                <input type="text" id="searchVal" name="searchVal" class="200" value="${searchVal}">
                <a href="#none" class="btn orange" id="btn_search"><span class="material-icons">search</span>검색</a>
            </div>
        </div>
    </article>
    </form>
    ...
    <article class="contUnit tbl_area">
        ...
        <table>
            ...
            <c:forEach items="${itemList}" var="item" varStatus="status">
                ...
                <tr>
                    <%--<td><input type="checkbox" name="del_id" id="del_id_${item.admin_id}" value="${item.admin_id}"></td>--%>
                    ...
                    <td>${item.grp_nm}</td>
                    <td><a href="/artmore_adm/cont/basic/admin_view.do?admin_id=${item.admin_id}">${item.name}</a></td>
                    <td><a href="/kihs_adm/cont/system/admin_view.do?admin_id=${item.admin_id}">${item.admin_id}</a></td>
        ...
        <div class="btnArea">
            ...
            <div class="right">
                <a href="/artmore_adm/cont/basic/admin_inscr.do" class="btn red">등록</a>
            </div>
        </div>
    </article>
</div>
